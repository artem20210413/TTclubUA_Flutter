...
Future<http.Response> REGISTRATION_LIST(String? token) async {
  final response =
      await http.get(Uri.parse(URL_REGISTATION_LIST), headers: HEADERS(token));

  return response;
}

Future<http.Response> REGISTRATION_COUNT(String? token) async {
  final response =
      await http.get(Uri.parse(URL_REGISTATION_COUNT), headers: HEADERS(token));
...
Future<http.Response> REJECT_REGISTRATION(int id, String? token) async {
  final response = await http.post(
      Uri.parse(
          URL_REGISTATION_CHANHE_ACTIVE.replaceAll('{id}', id.toString())),
