...
Future<http.Response> PARTNERS_LIST(
  String? token, {
  String? search,
  int page = 1,
  ...
  final uri = Uri.parse(URL_PARTNERS_LIST).replace(queryParameters: {
    'page': page.toString(),
...
Future<http.Response> PARTNERS_CREATE(String? token, PartnerDto dto) async {
  final response = await http.post(
    Uri.parse(URL_PARTNERS_CREATE),
...
Future<http.Response> PARTNERS_IMAGE_ADD(
...
Future<http.Response> PARTNERS_IMAGE_DELETE(
    String? token, int itemId, ImageUrlDto dto) async {
  final url = URL_PARTNERS_IMAGE_DELETE
      .replaceAll('{partner}', itemId.toString())
